...
export default function ForgotPasswordScreen() {
  ...
  const handleSendOtp = () => {
    ...
    if (!cleaned || cleaned.length < 6) { Alert.alert('提示', '請先輸入正確的手機號碼'); return; }
    setSendingOtp(true);
    sendOtpMutation.mutate({ areaCode: areaCode as '+853' | '+852' | '+86', phone: cleaned });
  };

  const handleSubmit = () => {
    const cleaned = phone.trim().replace(/\s/g, '');
    if (!cleaned || cleaned.length < 6) { Alert.alert('提示', '請輸入正確的手機號碼'); return; }
    ...
    setSubmitting(true);
    forgotMutation.mutate({ areaCode: areaCode as '+853' | '+852' | '+86', phone: cleaned, newPassword, otp });
  };

  const canSend = phone.trim().length >= 6 && countdown === 0 && !sendingOtp;
  const canSubmit = phone.trim().length >= 6 && newPassword.length >= 6 && newPassword === confirmPwd && otp.length === 6 && !submitting;
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.formWrap}>
          ...
          <View style={styles.phoneRow}>
            ...
            <TextInput
              style={styles.phoneInput} placeholder="請輸入手機號" placeholderTextColor={APP_GRAY}
